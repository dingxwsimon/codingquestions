...
public class Edge {
     ...
     * function. It uses a linear probe technique, which means in the case of a
     * collision, we just step forward through the table until we find the first
     * unused slot.
     */
    public void insert() {
    ...
    /**
     * Removing an edge from the hash table is a little more tricky. You have to
     * worry about creating a gap in the table that will make it impossible to
     * find other entries that have been inserted using a probe. Working around
     * this means that after setting an edge to be unused, we have to walk ahead
     * in the table, filling in gaps until all the elements can be found.
     * <p/>
    ...
    /**
     * The default ctor for Edge just sets start_node to the invalid value. This
     * is done to guarantee that the hash table is initially filled with unused
     * edges.
     */
    public Edge() {
        start_node = -1;
     ...
     * from the active point to the endpoint. Each time I create a new edge, I
     ...
     * have to split an existing edge. This function is called to split an edge
     * at the point defined by the Suffix argument. The existing edge loses its
     * parent, as well as some of its leading characters. The newly created edge
     * descends from the original parent, and now has the existing edge as a
     * child.
     * <p/>
