...
public class ReversePhrase {
    ...
    public static void Reverse(char[] chars) {
        ...
        Reverse(chars, 0, chars.length - 1);// notice we use 0 as start and last
        // index as end
        ...
        for (int i = 0; i < chars.length; i++) {
            // each time we check if char is space, if so, we reverse from
            // wordStart
            // until i-1
            if (chars[i] == ' ') {
            ...
            } else if (i == chars.length - 1)// there is a special case that
            // last char
            // is not space thus last word
            // won't be
            // reversed in previous if clause
            {
                Reverse(chars, wordStart, i);// this time the end index is i not
                // i-1
                // because this is end of phrase
            }
        }
    }

    // we firstly write the support method to reverse chars from stat to end
    // position
    ...
    private static void Reverse(char[] chars, int start, int end) {
        ...
        if (start >= end)
            return;
        // now we start from start until mid and swap with the other matched
        // char
        for (int i = start; i <= (start + end) / 2; i++)// loop stops at mid
        {
            char temp = chars[i];
            chars[i] = chars[start + end - i];// the index position start+end-i
            // is the
            // matched position!
