...
public class StringPermutation {
    ...
    public static void doCombination(char[] in, StringBuffer out,
	    boolean[] used, int length, int level, int k, int start) {
	if (level == k) {
	    System.out.println(out);
	    num++;
	    return;
    ...
    /*
     * public static ArrayList getPermutes(String s) { ArrayList permutes = new
     * ArrayList(); for(int i = 0; i < s.length(); i++) { char c = s.charAt(i);
     * String[] words = getPermutes(s.removeChar(c)); for (String word : words)
     * { permutes.append(c + word); } } return permutes; }
     */
