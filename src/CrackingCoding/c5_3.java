...
public class c5_3 {
	...
	/*
	 * Arithmetically: 2^c0 = 1 << c0 2^(c1-1) = 1 << (c0 - 1) next = n +
	 * 2^c0 + 2^(c1-1) - 1;
	 */
	...
	/*
	 * If temp is 0, then the number is a sequence of 0s followed by a
	 * sequence of 1s. This is already the smallest number with c1 ones.
	 ...
	 */
        ...
        int p = c0 + c1; // position of right-most non-trailing one (where the
        // right most bit is bit 0)

	/*
	 * Flip right-most non-trailing one. Example: n = 00011100011. c1 = 2 c0
	 * = 3 pos = 5
	 * 
	 * Build up a mask as follows: (1) ~0 will be a sequence of 1s (2)
	 * shifting left by p + 1 will give you 11.111000000 (six 0s) (3) ANDing
	 * with n will clear the last 6 bits n is now 00011000000
	 */
        n &= ((~0) << (p + 1)); // clears from bit p onwards (to the right)

	/*
	 * Create a sequence of (c1+1) 1s as follows (1) Shift 1 to the left
	 * (c1+1) times. If c1 is 2, this will give you 0..001000 (2) Subtract
	 * one from that. This will give you 0..00111
	 */
        int mask = (1 << (c1 + 1)) - 1; // Sequence of (c1+1) ones

	/*
	 * Move the ones to be right up next to bit p Since this is a sequence
	 * of (c1+1) ones, and p = c1 + c0, we just need to shift this over by
	 * (c0-1) spots. If c0 = 3 and c1 = 2, then this will look like
	 * 00...0011100
	 * 
	...
	/*
	 * Arithmetic: 2^c1 = 1 << c1 2^(c0 - 1) = 1 << (c0 - 1)
	 */
