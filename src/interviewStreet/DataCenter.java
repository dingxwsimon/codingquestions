...
public class DataCenter
{
  ...
  static final int NULL = -1, OPEN = 0, BLOCKED = 1, START = 2, END = 3,
      XSHIFT = 3, YMASK = (1 << XSHIFT) - 1, // SHIFT needs to be increased if X
                                             // or Y need more than SHIFT bits
      D[] = new int[] { 1, -1, -(1 << XSHIFT), 1 << XSHIFT }, // deltas for
                                                              // Left, Right,
                                                              // Up, Down
      CACHE_SIZE = 1 << 16;
  ...
  static int remaining, start, end, // last SHIFT bits are y; x is stored in
                                    // SHIFT bits on the left of y.
      X, Y;

  static long state; // use 64-bits to represent upto 64 cells, always make sure
                     // you use 1L when shifting, for large cases use BitSet
  ...
  static int compute(final int p)
  {
    ...
    try {
      ...
      if (numOfExits(end) == 0 || !isFloodFillable()) // TODO: smart schedule
                                                      // floodfills - maybe more
                                                      // later or after 3
                                                      // running sum left turns?
        return cacheAndReturn(state, p, remaining == 1
                                                      ? 1
                                                      : 0);
      ...
      for (int d : D) {
        if (matrix[d += p] && d != end && numOfExits(d) == 1) {
          if (onlyChoice != NULL) return cacheAndReturn(state, p, 0);
          onlyChoice = d;
        }
      }
  ...
  static boolean isFloodFillable()
  {
    ...
    for (int head = 0 ; tail > head ; head++) {
      p = queue[head];
      for (int d : D)
        if (matrix[d += p]) matrix[queue[tail++] = d] = !VISITABLE;
    }
