...
public class ApplePass {
     ...
     * hand. You find that the total number of the apples can be divided by N.
     * So you want to divide the apples equally among all the boys. But they are
     * so lazy that each one of them only wants to give one apple to one of the
     * neighbors at one step. Calculate the minimal number of steps to make each
     * boy have the same number of apples. Input Given: 1. A number N => number
     * of children. 2. Sequence of N numbers, each representing number of apples
     * a child has. <<P.S.>> Passing an apple means a child giving away one
     * apple to one of its neighbour. Even if 2 separate children can pass
     * apples simultaneously or one child can pass 1-1 apple to each of its
     * neighbours then that will still be counted as 2 steps and not 1 step.
     */
	...
	for (int i = 0; i < childCount; ++i) {
	    ...
	    // If the child before us has less apples than desired, pass it to
	    // him
