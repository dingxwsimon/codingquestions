...
public class LIS {
    ...
    // basically the same n^2 method
    public static void printLIS(int[] nums) {
	// as we discussed in slides, in order to use DP in this example, we
	// need
	// 1. a size array to keep track of longest LIS ending with current
	// position
	// 2. an accordingly string array to keep track of the path for printing
	// out
	String[] paths = new String[nums.length];
	int[] sizes = new int[nums.length];

	// firstly we assign the intial values to each path/size, by setting
	// size
	// to 1 and path equals the value (that means initially each path
	// starting/ending
	...
	int maxLength = 1;// notice it is 1, but it really does not matter if
			  // sets
			  // to 0

	for (int i = 1; i < nums.length; i++) {
	    // notice we start from 2nd postion, 1st position is no point
	    for (int j = 0; j < i; j++) { // the inner loop is to check all
					  // previous
					  // items!
		// now it's the key step,
		// when do we append our current index to the previous
		// subsequence?
		// it has to meet 2 requirements,
		// current>previous ending and size is increasing!
		if (nums[i] > nums[j] && sizes[i] < sizes[j] + 1) { // notice
								    // plus
								    // one!!
		    // if so, we need update sizes and path
		    sizes[i] = sizes[j] + 1;
		    paths[i] = paths[j] + nums[i] + " ";// append current values
							// to end!
	...
	// finally go scanning the size array again and print out the path when
	// size
	// matches
	for (int i = 1; i < nums.length; i++) {
