...
public class LIS
{
  ...
  public static void printLIS(int[] nums)
  {
    ...
    int maxLength = 1;// notice it is 1, but it really does not matter if sets
                      // to 0

    for (int i = 1 ; i < nums.length ; i++) {
      // notice we start from 2nd postion, 1st position is no point
      for (int j = 0 ; j < i ; j++) { // the inner loop is to check all previous
                                      // items!
    ...
    // finally go scanning the size array again and print out the path when size
    // matches
    for (int i = 1 ; i < nums.length ; i++) {
      if (sizes[i] == maxLength) System.out.println("LIS: " + paths[i]);
    }
