...
public class DecisionTree
{
  ...
  public static void main(String[] args)
  {
    ...
    examples[0] = new Example(example_attributes, gr_20, gr_50, no, no,
        scattered, yes);
    ...
    examples[3] = new Example(example_attributes, gr_20, gr_50, gr_80, no,
        overcast, yes);
    examples[4] = new Example(example_attributes, gr_20, gr_50, gr_80, yes, no,
        yes);
    examples[5] = new Example(example_attributes, gr_20, gr_50, gr_80, yes,
        overcast, yes);
    ...
    examples[8] = new Example(example_attributes, gr_20, no, no, no, overcast,
        yes);
    examples[9] = new Example(example_attributes, gr_20, no, no, no, scattered,
        yes);

    Attribute desired_attribute = example_attributes[example_attributes.length - 1]; // desired
                                                                                     // attribute
                                                                                     // is
                                                                                     // the
                                                                                     // last

    Node<?> tree = learnDecision(examples, real_attributes, yes,
        desired_attribute);
   ...
   * @param default_label list of default condition for desired attribute to
   *        apply
   * @param desired_attribute the attribute to test for
   * @return
   */
  public static Node<?> learnDecision(Example[] examples,
      Attribute[] attributes, Condition default_label,
      Attribute desired_attribute)
  {
    ...
    // recurse
    Attribute best = ChooseBestAttribute(examples, attributes,
        desired_attribute);
    ...
    for (Condition c : best.possible_conditions) {
      ...
      Node<?> sub_tree = learnDecision(example_i, removeBest(attributes, best),
          label, desired_attribute);
  ...
  public static Attribute[] removeBest(Attribute[] attributes, Attribute best)
  {
    ...
    for (Attribute a : attributes) {
      if (!a.equals(best)) modified_attributes.add(a);
    }
  ...
  public static Condition Mode(Example[] examples)
  {
    ...
    for (Example e : examples) {
      ...
      for (Example inner_e : examples) {
        if (inner_e.get_label().equals(e.get_label())) local_count++;
      }
  ...
  /**
   * This will calculate the Gain and Remainder of attributes and picks the best
   * to recurse over
   * 
   * @param examples
   * @param attributes
   ...
   * @return
   */
  public static Attribute ChooseBestAttribute(Example[] examples,
      Attribute[] attributes, Attribute desired_attribute)
  {
   ...
   */
  public static double Remain(Example[] examples, Attribute attribute,
      Attribute desired_attribute)
  {
    ...
    for (Condition major_condition : attribute.possible_conditions) {
      ...
      for (Condition c : desired_attribute.possible_conditions) {
        ...
        System.out.println("number of passing sub examples: "
            + examples_c.length);
  ...
  public static double IGain(Double... ds)
  {
    ...
    for (double d : ds) {
      if (d != 0.0) final_value += -d * Math.log(d) / Math.log(2.0);
    }

    if (Double.isNaN(final_value)) final_value = 0;
  ...
  public static class Attribute
  {
    ...
    public Example[] satisfied(Example[] examples, Condition c)
    {
      ...
      for (Example e : examples) {
        if (e.check_condition(this, c)) satisfied_examples.add(e);
      }
  ...
  public static class Node<T>
  {
    ...
    public String toString(String tabs)
    {
      ...
      if (childs.equals("")) childs = "no children.";

      return "\n" + tabs + "Node: " + identifier + " value: " + data.toString()
          + " children: " + childs;
    }
